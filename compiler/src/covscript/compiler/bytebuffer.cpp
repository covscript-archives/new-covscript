...
namespace cs {
    namespace compiler {
        ...
        ByteBuffer::ByteBuffer(size_t initialLength)
            : buffer(nullptr), bufferLength(initialLength), position(0),
        ...
        void ByteBuffer::growIfNeeded(size_t least) {
            ...
            while (position + least >= bufferLength) {
                growBuffer();
            }
        }
        ...
        int16_t ByteBuffer::readInt16At(int index) {
            return (byteOrder == BO_BIG_ENDIAN ? readInt16AtBE(index) : readInt16AtLE(index));
        }

        int32_t ByteBuffer::readInt32At(int index) {
            return (byteOrder == BO_BIG_ENDIAN ? readInt32AtBE(index) : readInt32AtLE(index));
        }

        int64_t ByteBuffer::readInt64At(int index) {
            return (byteOrder == BO_BIG_ENDIAN ? readInt64AtBE(index) : readInt64AtLE(index));
        }

        float ByteBuffer::readFloatAt(int index) {
            return (byteOrder == BO_BIG_ENDIAN ? readFloatAtBE(index) : readFloatAtLE(index));
        }

        double ByteBuffer::readDoubleAt(int index) {
            return (byteOrder == BO_BIG_ENDIAN ? readDoubleAtBE(index) : readDoubleAtLE(index));
        }
        ...
        void ByteBuffer::writeInt16At(int index, int16_t value) {
            growIfNeeded(2);
            (byteOrder == BO_BIG_ENDIAN) ? writeInt16AtBE(index, value) : writeInt16AtLE(index, value);
        }

        void ByteBuffer::writeInt32At(int index, int32_t value) {
            growIfNeeded(4);
            (byteOrder == BO_BIG_ENDIAN) ? writeInt32AtBE(index, value) : writeInt32AtLE(index, value);
        }

        void ByteBuffer::writeInt64At(int index, int64_t value) {
            growIfNeeded(8);
            (byteOrder == BO_BIG_ENDIAN) ? writeInt64AtBE(index, value) : writeInt64AtLE(index, value);
        }

        void ByteBuffer::writeFloatAt(int index, float value) {
            growIfNeeded(4);
            (byteOrder == BO_BIG_ENDIAN) ? writeFloatAtBE(index, value) : writeFloatAtLE(index, value);
        }

        void ByteBuffer::writeDoubleAt(int index, double value) {
            growIfNeeded(8);
            (byteOrder == BO_BIG_ENDIAN) ? writeDoubleAtBE(index, value) : writeDoubleAtLE(index, value);
        }

        void ByteBuffer::writeStringAt(int index, const std::string &value) {
            growIfNeeded(value.length());
            copy(reinterpret_cast<const byte *>(value.c_str()), buffer + index, value.length());
        }
        ...
        int32_t ByteBuffer::readInt32AtBE(int index) {
            byte b0 = buffer[index + 3];
            byte b1 = buffer[index + 2];
        ...
        int64_t ByteBuffer::readInt64AtLE(int index) {
            byte b7 = buffer[index + 7];
            byte b6 = buffer[index + 6];
            byte b5 = buffer[index + 5];
            byte b4 = buffer[index + 4];
            byte b3 = buffer[index + 3];
            byte b2 = buffer[index + 2];
            byte b1 = buffer[index + 1];
            byte b0 = buffer[index];
        ...
        int64_t ByteBuffer::readInt64AtBE(int index) {
            byte b0 = buffer[index + 7];
            byte b1 = buffer[index + 6];
            byte b2 = buffer[index + 5];
            byte b3 = buffer[index + 4];
            byte b4 = buffer[index + 3];
            byte b5 = buffer[index + 2];
            byte b6 = buffer[index + 1];
            byte b7 = buffer[index];
